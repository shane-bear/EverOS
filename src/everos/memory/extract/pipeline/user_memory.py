    ...
    async def run(
        ...
        for cell, memcell_id, all_senders in zip(
            ...
            with memory_span(
                ...
                #
                # Retry malformed output and transient provider failures before
                # the episode is persisted. The latter commonly surface as a
                # generic LLMError after the provider's own request retries.
...
async def _extract_with_retry(
    ...
    """Call episode extraction with retry before any episode is persisted."""
    ...
    # Final attempt — an extraction failure here propagates to the caller.
    return await extractor.aextract(cell, sender_id=None, prompt=prompt)
